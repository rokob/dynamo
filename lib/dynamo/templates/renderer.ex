defmodule Dynamo.Templates.Renderer do
  ...
  @doc false
  def handle_call({ :get_module, identifier, updated_at }, _from, { name, map }) do
    case Map.get(map, identifier) do
      { module, cached } when updated_at > cached ->
        spawn fn -> purge_module(module) end
        { :reply, generate_suggestion(name, 0), { name, Map.delete(map, identifier) } }
      { module, _ } ->
        { :reply, { :ok, module }, { name, map } }
      nil ->
        { :reply, generate_suggestion(name, 0), { name, map } }
    end
  end
